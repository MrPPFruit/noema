...
- 当前公开版本线：`v0.1.0-beta.2`
- Release 类型：GitHub pre-release
- 当前公开平台：Android
- 当前 release：[Noema 0.1 Beta 2](https://github.com/MrPPFruit/noema/releases/tag/v0.1.0-beta.2)
...
- 不默认后台扫描全图库。
- 不自动删除系统相册原照片；只有你明确选择删除原图并通过系统确认后，Noema 才会同步移除。
- 不默认上传照片到云端。
- 不提供内置 AI API Key。
- 不把公开仓库当作照片素材包或二创模板。
...
| 自动删除原照片 | 否 | 只有用户明确选择删除原图并通过系统确认后才会执行 |
...
- [Noema 0.1 Beta 2](https://github.com/MrPPFruit/noema/releases/tag/v0.1.0-beta.2)
